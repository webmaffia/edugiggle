...
type WebinarsData = {
  heading: string;
  events: { day: string; month: string; title: string; when: string }[];
};

const DEFAULT_DATA: WebinarsData = {
  heading: "Upcoming Webinars & Events",
  events: [
...
export default async function Webinars() {
  const data = await readSection<WebinarsData>("home.webinars", DEFAULT_DATA);

  return (
    <section className="py-20 bg-surface">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-8">
          <h2 className="text-2xl md:text-3xl font-extrabold text-secondary">{data.heading}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {data.events.map((event, i) => (
            <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-start gap-4">
